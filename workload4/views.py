...
def workload_list(request,id=None):
	...
	if request.user.is_staff or request.user.is_superuser:
		...
		queryset = Document.objects.filter(user=current_user)
...
def workload_report(request):
	...
	if not request.user.is_authenticated :
		return redirect("login")
	data = Document.objects.all().values('user__username').annotate(sum_items=Sum('num_of_lecture'))
   	for instance in data:
   		print(instance)
	return render(request, "workload4/workload_report.html")
