...
def workload_report(request):
	...
	if not request.user.is_authenticated :
		return redirect("login")
	data = Benefit.objects.all().values('user__username').annotate(sum_items=Sum('num_of_lecture'))
   	for instance in data:
   		print(instance)
	return render(request, "workload7/workload_report.html")
...
def sum_report(request):
	data = Position.objects.all().values('user__username').annotate(sum_items=Sum('num_of_lecture'))
